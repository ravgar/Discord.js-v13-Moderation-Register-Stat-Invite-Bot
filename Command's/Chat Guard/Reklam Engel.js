...
module.exports = { name: "reklamengel", aliases: ["reklamEngel"],   category: "Chat-Guard", desc: "---",
    execute: async (client, message, args, FlatEmbed) => { 
      ...
      if(args[0].toLowerCase() === "aç") {
        await ChatGuard.findOneAndUpdate({guildID: client.guilds.cache.get(Settings.guildID) }, { reklamEngel: true }, {upsert: true});
        message.reply(`${client.emojis.cache.find(x => x.name === "ravgar_tik")} Reklam Koruması Başarılı Bir Şekilde Aktif edildi.`).then(e => setTimeout(() => e.delete().catch(() => { }), 10000))

      } else if (args[0].toLowerCase() === "kapat") {
        await ChatGuard.findOneAndUpdate({guildID: client.guilds.cache.get(Settings.guildID) }, { reklamEngel: false }, {upsert: true});
        message.reply(`${client.emojis.cache.find(x => x.name === "ravgar_tik")} Reklam Koruması Başarılı Bir Şekilde De-Aktif edildi.`).then(e => setTimeout(() => e.delete().catch(() => { }), 10000))
