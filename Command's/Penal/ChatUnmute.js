...
module.exports = { name: "uncmute", aliases: ["unmute", "cunmute"],  category: "Penal", desc: "Kullanıcının Ses Kanallarındaki Susturmasını Kaldıran Komut",
    ...
    if (!User.roles.cache.has(Server.vmutedRole)) return message.reply(AutoReply.CmuteDisable).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "ravgar_carpi")}`)
    ...
    message.reply(`${client.emojis.cache.find(x => x.name === "ravgar_tik") || "Emoji Bulunamadı"} ${User} (\`${User.user.tag}\` - \`${User.id}\`) isimli kullanıcının, metin kanallarındaki susturulması ${message.author} tarafından kaldırıldı.`).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "ravgar_tik")}`)
}}